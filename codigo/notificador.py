...
A URL do webhook é resolvida nesta ordem (a 1ª que existir vence):
  ...
  3. `WEBHOOK_PADRAO` abaixo (no código).

ATENÇÃO: a URL do webhook contém o token `sig=` — é uma credencial. Neste
repositório público `WEBHOOK_PADRAO` fica vazio; configure via env ou arquivo.
Se a URL vazar, recrie o fluxo no Power Automate (gera nova URL).
...
# Webhook padrão (Power Automate Workflow). Vazio no repositório público:
# configure via env TEAMS_WEBHOOK_URL ou arquivo teams_webhook.url (a URL
# contém o token `sig=` e não deve ser versionada).
WEBHOOK_PADRAO = ""

# Pasta do SharePoint onde o fluxo salva o CSV do dia (a MESMA do Create file).
# Usada para montar o botão "Abrir <arquivo>" no card; sem ela, o card sai sem botão.
