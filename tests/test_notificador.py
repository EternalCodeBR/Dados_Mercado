...
def test_payload_com_arquivo_monta_botao_abrir(monkeypatch):
    monkeypatch.setattr(notificador, "SHAREPOINT_HISTORICO_URL", "https://exemplo.sharepoint.com/pasta/")
    ...
    assert acao["url"].startswith("https://exemplo.sharepoint.com/")
    assert p["arquivo_nome"] == "Dadosmercado_230626.csv"


def test_payload_com_arquivo_sem_sharepoint_url_nao_tem_botao(monkeypatch):
    monkeypatch.setattr(notificador, "SHAREPOINT_HISTORICO_URL", "")
    p = notificador._payload(
        "https://x.logic.azure.com/w", "T", "corpo", True, "Dadosmercado_230626.csv"
    )
...
def test_url_sem_env_e_sem_arquivo_retorna_none(monkeypatch, tmp_path):
    monkeypatch.delenv("TEAMS_WEBHOOK_URL", raising=False)
    monkeypatch.setattr(notificador.paths, "TEAMS_WEBHOOK_FILE", tmp_path / "nao_existe.url")
    assert notificador._url() is None


def test_env_tem_prioridade_sobre_arquivo(monkeypatch, tmp_path):
    ...
    monkeypatch.delenv("TEAMS_WEBHOOK_URL", raising=False)
    assert notificador._url() == "https://x.logic.azure.com/do-arquivo"  # arquivo, sem env
